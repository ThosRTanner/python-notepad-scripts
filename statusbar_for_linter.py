...
from commctrl import STATUSCLASSNAME, SB_GETPARTS, SB_GETTEXT, SB_GETTEXTLENGTH, SB_SETPARTS
...
# It seems wrong getthing the notepad++ window and wandering down it.
...
def read_statusbar_section(handle, section):
    ...
    ret = SendMessage(handle, SB_GETTEXT, section, val)
    return val.value
...
def override_status_bar(args):
    ...
    if not val.startswith(" - ") or val == " - ":
        if parts is not None:
            SendMessage(statusbar_handle, SB_SETPARTS, len(parts), parts)
            parts = None
    else:
        if parts is None:
            ...
            res = SendMessage(statusbar_handle, SB_GETPARTS, num_parts, parts)
            ...
            SendMessage(statusbar_handle, SB_SETPARTS, len(nparts), nparts)
